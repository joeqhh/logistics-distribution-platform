...
export const createOrderHandler = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return successResponse(res, { order: newOrder, logistics: newLogistics }, '订单创建成功');
  } catch (error) {
...
// 获取用户订单列表
export const getConsumerOrdersHandler = async (req: AuthRequest, res: Response) => {
  try {
    const consumerId = req.user?.id;
...
// 获取商家订单列表
export const getMerchantOrdersHandler = async (req: AuthRequest, res: Response) => {
  try {
...
// 获取订单详情
export const getOrderDetailHandler = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
...
// 更新订单状态（发货等操作）
export const updateOrderStatusHandler = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    ...
    // 更新订单状态
    const updatedOrder = await updateOrder(id, { status: status as OrderStatus });
