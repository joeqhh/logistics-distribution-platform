...
import useRoute, { type IRoute } from '@/routes'
...
function getFlattenRoutes(routes: IRoute[]) {
  const mod: any = import.meta.glob('@/views/user/**/[a-z[]*.tsx')
  ...
  function travel(_routes: IRoute[]) {
    _routes.forEach((route) => {
      if (route.key && !route.children) {
        ...
        route.component = lazyload(mod[`/src/views/${route.key}/index.tsx`])
...
function PageLayout() {
  ...
  useEffect(() => {
    ...
    //设置路由的name为当前页面的title
    if (routeConfig && routeConfig.length > 0) {
      const title = routeConfig[routeConfig.length - 1]
      document.title = (typeof title === 'string' ? title : '') + ' - 电商物流配送平台'
    }
                ...
                <div className={styles['layout-breadcrumb']}>
                  <Breadcrumb>
                    <Breadcrumb.Item key={-1}>
                      {getIconFromKey(pathname.slice(1))}
                    </Breadcrumb.Item>
